...
export async function GET(request: NextRequest) {
	...
	try {
		...
		if (!canAccessUserResource(tokenResult.payload.userId, userId)) {
			console.log(
				'[LINKS API] Authorization failed: user',
				tokenResult.payload.userId,
				'cannot access handles for user',
				userId
			);
			return NextResponse.json(
				{ success: false, error: 'Forbidden: You can only access your own handles' },
				{ status: 403 }
			);
		}
	...
	try {
		...
		if (!canAccessUserResource(tokenResult.payload.userId, userId)) {
			console.log(
				'[LINKS API] Authorization failed: user',
				tokenResult.payload.userId,
				'cannot create handle for user',
				userId
			);
			return NextResponse.json(
				{ success: false, error: 'Forbidden: You can only create handles for yourself' },
				{ status: 403 }
			);
		}
		...
		return NextResponse.json({ success: false, error: 'Failed to create handle' }, { status: 500 });
	} finally {
		console.log('[LINKS API] Closing database pool');
		await pool.end();
	}
...
export async function PUT(request: NextRequest) {
	...
	try {
		...
		if (!handleId || typeof isEmbedded !== 'boolean') {
			...
			return NextResponse.json(
				{ success: false, error: 'Handle ID and embed status are required' },
				{ status: 400 }
			);
		}
		...
		if (ownershipCheck.rows.length === 0) {
			...
			return NextResponse.json({ success: false, error: 'Handle not found' }, { status: 404 });
		}
		...
		if (!canAccessUserResource(tokenResult.payload.userId, handleOwnerId)) {
			console.log(
				'[LINKS API] Authorization failed: user',
				tokenResult.payload.userId,
				'cannot modify handle owned by user',
				handleOwnerId
			);
			return NextResponse.json(
				{ success: false, error: 'Forbidden: You can only update your own handles' },
				{ status: 403 }
			);
		}
		...
		return NextResponse.json({ success: false, error: 'Failed to update handle' }, { status: 500 });
	} finally {
		console.log('[LINKS API] Closing database pool');
		await pool.end();
	}
