...
class CallSafeWidget {
  constructor(config, scriptElement) {
    ...
    // TURN credentials (pre-fetched in background)
  ...
  async initiateCall(type = 'voice') {
    ...
    try {
      ...
      // Override remote stream handling to use embed's own DOM pattern
      ...
      // Send call:initiate — transport is connected at this point
      const initiateData = {
  ...
  handleRemoteStream(stream, callType) {
    if (callType === 'video') {
      ...
      // Create audio element dynamically (no audio element in modal HTML)
  ...
  emit(event, data) {
    ...
    if (listeners) {
      listeners.forEach(callback => {
        try { callback(data); } catch (error) { console.error('CallSafe: Event listener error', error); }
      });
    }
